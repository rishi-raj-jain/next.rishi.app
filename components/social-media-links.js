...
const SocialMediaLinks = ({ className, url, altText }) => {
  ...
  return (
    <div className={`${className ?? ''} flex flex-row space-x-3 text-black dark:text-white`}>
      <a
        target="_blank"
      ...
      >
        <span
          className={`absolute top-8 -right-8 w-[120px] rounded bg-gray-100 px-2 py-1 text-center text-sm text-black transition-transform duration-500 ${
            ifLink ? 'block' : 'hidden'
          }`}
        >
          Link Copied!
